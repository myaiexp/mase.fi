...
- **Member convention:** Inside a component class, `#member` is hard-private internal state; `_member` marks a member deliberately reachable by a **friend module or test**. Default to `#`; use `_` only when something outside the class must read it. Current `_` members: `select._menu` / `select._selectOption` (read by the friend module `select-menu.js`) and `component._overlayListeners` (stamped on by `shared/overlay-utils.js`). The three overlay components (`context-menu`, `dropdown`, `select`) follow this; each states it at the top of its class.
